...
Removes duplicate sequences, which may be specified to be exact matcheProportion, subsequences, or sequences within some percent identity.
...
absorbmatch=t         (am) Absorb exact matcheProportion of contigs.
...
exact=t               (ex) Only allow exact symbol matcheProportion.  When false, an 'N' will match any symbol.
...
hashns=f              Set to true to search for matcheProportion using kmers containing Ns.  Can lead to extreme slowdown in some cases.
#ignoreaffix1=f       (ia1) Ignore first affix (for testing).
#storesuffix=f        (ss) Store suffix as well as prefix.  Automatically set to true when doing inexact matcheProportion.
