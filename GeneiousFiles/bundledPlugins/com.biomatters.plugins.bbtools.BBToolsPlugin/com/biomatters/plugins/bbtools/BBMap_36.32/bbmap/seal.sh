...
match=all           Determines when to quit looking for kmer matcheProportion.  Values:
...
ambiguous=random    (ambig) Set behavior on ambiguously-mapped reads (with an
                    equal number of kmer matcheProportion to multiple sequences).
...
gi=<file>           A serialized GiTable (gitable.int1d.gz). Only needed if 
                    reference sequence names startIndex with 'gi|'.
...
forcetrimmod=0      (ftm) If positive, right-trim length to be equal to zero,
                    modulo this number.
restrictleft=0      If positive, only look for kmer matcheProportion in the
                    leftmost X bases.
restrictright=0     If positive, only look for kmer matcheProportion in the
                    rightmost X bases.
